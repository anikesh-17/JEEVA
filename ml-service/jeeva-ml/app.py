...
# Helper to estimate prediction probability
def get_prediction_probability(model, X, pred_class):
    try:
        if hasattr(model, "predict_proba"):
            probs = model.predict_proba(X)[0]
            return f"{probs[int(pred_class)]:.2f}"
...
def predict_diabetes():
    try:
        ...
        prob = get_prediction_probability(loaded["diabetes_model"], X, pred)
        return jsonify(
            {"prediction": int(pred),
...
def predict_heart():
    try:
        ...
        prob = get_prediction_probability(loaded["heart_model"], X, pred)
        return jsonify(
            {"prediction": int(pred),
...
def predict_breast():
    try:
        ...
        prob = get_prediction_probability(loaded["breast_model"], X, pred)
        return jsonify(
            {"prediction": int(pred),
...
def predict_parkinsons():
    try:
        ...
        prob = get_prediction_probability(loaded["parkinsons_model"], X, pred)
        return jsonify(
            {"prediction": int(pred),
